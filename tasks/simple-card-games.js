...
function winner(deckSteve, deckJosh) {
  deckSteve = normalize(deckSteve);
  deckJosh = normalize(deckJosh);
  
  let steveWin = 0;
  let joshWin = 0;
  for (let i = 0; i < deckSteve.length; i++) {
    if (deckSteve[i] === deckJosh[i]) continue;
  ...
  return steveWin > joshWin ? `Steve wins ${steveWin} to ${joshWin}` : steveWin < joshWin ? `Josh wins ${joshWin} to ${steveWin}` : 'Tie'
}

console.log("===== SIMPLE CARD GAMES =====");
